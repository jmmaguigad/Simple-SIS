...
if ($id == "" && isset($_SESSION['user_session'])) {
  ...
  $data = 'tblfaculty','tblaccessinfo','tblfaculty.access_id = tblaccessinfo.record_num',"tblfaculty.record_num_fac = $user_id",1,'Inner';
} else {
  $data = 'tblfaculty','tblaccessinfo','tblfaculty.access_id = tblaccessinfo.record_num',"tblfaculty.record_num_fac = $id",1,'Inner';
}
foreach ($database->selectData($data) as $rowFac) {
?>
